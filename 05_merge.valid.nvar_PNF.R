...
rm(list = ls())
dir_PGS <- "/projects/rpci/songyao/pnfioric/pgs_calc/breast_cancer_pgs/pgs_weights"
...
PGS.scoreNM <- PGS.scoreID #c("Breast_Cancer_313", "AABCGS_black", "Gao2022_BrCa", "Gao2022_ERNEG", "Gao2022_ERPOS", "Shieh2023")
...
for (i in 1:length(PGS.scoreID)) {
  for (pop in c("AA", "EA4", "EU", "HIS")) {
    mywd = paste("/projects/rpci/songyao/pnfioric/pgs_calc/breast_cancer_pgs/", PGS.scoreID[i], "/",  pop, "/", sep = '')
...
library(openxlsx)
write.xlsx(df_PGS.sm_Pathway, "/projects/rpci/songyao/pnfioric/pgs_calc/breast_cancer_pgs/PGS.matching.summary_Pathways_BrCa_PGS330.xlsx", colWidths = "auto")
write.csv(df_PGS.sm_Pathway, "/projects/rpci/songyao/pnfioric/pgs_calc/breast_cancer_pgs/PGS.matching.summary_Pathways_BrCa_PGS330.xlsx", row.names = F)

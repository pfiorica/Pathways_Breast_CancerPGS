...
for (i in 1:length(PGS.scoreID)) {
  ...
  names(df_ls[[i]])[2] <- paste0("", PGS.scoreNM[i])
...
library(openxlsx)
write.xlsx(df_PGS_Pathway, "/projects/rpci/songyao/pnfioric/pgs_calc/breast_cancer_pgs/Pathways_330_PGS.xlsx", colWidths = "auto")
write.csv(df_PGS_Pathway, "/projects/rpci/songyao/pnfioric/pgs_calc/breast_cancer_pgs/Pathways_330_PGS.csv", row.names = F)
